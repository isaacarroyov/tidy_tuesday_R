...
df <- artists %>%
  ...
  replace_na(list(artists_share = 0, location_quotient = 0)) %>%
  mutate(artists_share_perc = artists_share * 100,
         concentration_labor_force = (location_quotient - 1)*100,
         art_expression = case_when(type %in% c("Designers", "Fine Artists, Art Directors, And Animators", "Photographers") ~ "Visual",
         ...
         art_expression = case_when(art_expression == "Architecture" ~ "Architecture (Architects and Landscape Architects)",
                                           ...
                                           T ~ "Another art expression")) %>%
  rename(state_usa = state) %>%
  group_by(state_usa, art_expression) %>%
  summarise(avg_artists_share_perc = mean(artists_share_perc),
            avg_concentration_labor_force = mean(concentration_labor_force)) %>%
  ungroup()
