...
# Data Visualization -> bubble plot + curves
...
# Info and lines that link info tu curves
p2 <- p1 + geom_curve(aes(x = xpos, xend = xpos,
  ...
  geom_textbox(aes(label = label, x = xpos, y = if_else(condition = xpos%%2 == 0, true = max(max_height_view) + 100, false = max(max_height_view) + 250)),
               ...
               width = unit(60,"pt"), fill = '#FFFCFB',
               box.padding = margin(2,2,2,2), box.r = unit(3,'pt'),
               )
# Annotations
p3 <- p2 + geom_textbox(data = tribble(~x,~y,~label,
                        ...
                        aes(x=x,y=y,label=label), fill = NA, box.colour = NA, width = unit(6,'in'),
...
p6 <- p5 + theme(
  ...
  panel.background = element_rect(fill = '#FFFAF9', colour = '#FFFAF9'),
  # Title
  plot.title.position = 'plot',
  plot.title = element_textbox_simple(family = 'englishFont', size = 85, margin = margin(t = 15, b = 10, l = 10)),
  # Subtitle
  plot.subtitle = element_textbox_simple(family = 'englishFont', size = 40, lineheight = 0.4, margin = margin(b = 15, l = 10)),
  # CAption
  plot.caption.position = 'plot',
  plot.caption = element_textbox_simple(family = 'englishFont', face = 'bold', size = 30, lineheight = 0.5, margin = margin(b = 15, l = 10)),
  )
